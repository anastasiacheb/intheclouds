...
import { motion, useScroll, useTransform, motionValue, useInView, useAnimate } from 'motion/react';
...
function SlideOne({ progress }) {
  ...
  useEffect(() => {
    const duration = 40; // количество кадров
    let frame = 0;

    // небольшой задержка перед стартом (например, 200мс)
    const delayTimeout = setTimeout(() => {
      const interval = setInterval(() => {
        frame++;

        // плавное движение по Y
        yInitial.set(200 * (1 - frame / duration));

        // плавная прозрачность от 0 до 1
        opacityInitial.set(frame / duration);
  ...
  return (
    <>
      <div className="absolute top-[13vh] left-1/2 -translate-x-1/2 h-[55vh] w-full">
        <motion.div style={{ y: yPlanet }} className="w-full flex justify-center">
          <Image
      ...
      <motion.div style={{ y: yLand }}>
        <Image
          ...
          height={0}
          sizes="100vw"
          className="h-[31vh] w-auto max-h-64  absolute bottom-0 left-1/2 -translate-x-1/2 object-cover scale-120"
        />
      </motion.div>
      <motion.div style={{ y: yPeople }}>
        <Image
          ...
          height={0}
          sizes="100vw"
          className="h-[37vh] w-auto max-h-75 absolute bottom-8 left-1/2 -translate-x-1/2 object-contain scale-120"
        />
      </motion.div>
...
function SlideThree({ progress }) {
  ...
  return (
    <>
      ...
      <div className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 h-full max-h-160">
...
function SlideFour({ progress }) {
  ...
  return (
    <>
      ...
      <div className="absolute top-[32vh] right-1/2 -translate-x-90 ">
      ...
      <div className="h-auto absolute top-[18vh] right-1/2 -translate-x-75">
        <motion.div style={{ y: yOther }}>
          <Image
            src={`/images/ufo.png`}
            alt="background"
      ...
      <div className="absolute top-[8vh] left-1/2 translate-x-70">
        <motion.div style={{ y: yOther }}>
          <Image
            src={`/images/ufo.png`}
            alt="background"
      ...
      <div className="absolute top-[9vh] left-1/2 -translate-x-1/2">
        <motion.div style={{ y: yOther }}>
          <div className=" w-max flex flex-col items-center">
            ...
            <Image
              ...
              height={0}
              sizes="100vw"
              className="h-[66vh] w-auto max-h-133 object-cover px-2"
            />
          </div>
        </motion.div>
      </div>
      <motion.div style={{ y: yLand }}>
        <Image
          ...
          height={0}
          sizes="100vw"
          className="h-[44vh] w-auto absolute bottom-0 left-1/2 -translate-x-1/2 object-cover"
        />
      </motion.div>
...
function Slide({ backgroundImage, children }) {
  ...
  useEffect(() => {
    if (isInView) {
      animate(scope.current, { scale: 1 }, { duration: 3 });
    } else {
